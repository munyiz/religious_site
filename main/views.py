...
def questions_list(request):
    if request.user.is_authenticated and request.user.is_staff:
        ...
        questions = (
            Question.objects.filter(private=False)
            | Question.objects.filter(user=request.user)
            | Question.objects.filter(authorized_users=request.user)
        )
    else:
...
def view_question(request, question_id):
    ...
    if question.private and not (
        request.user == question.user or request.user.is_staff or request.user in question.authorized_users.all()
    ):
...
def stories_list(request):
    if request.user.is_authenticated and request.user.is_staff:
        ...
        stories = (
            Story.objects.filter(private=False)
            | Story.objects.filter(user=request.user)
            | Story.objects.filter(authorized_users=request.user)
        )
    else:
...
def like_post(request, post_type, post_id):
    ...
    like, created = Like.objects.get_or_create(
        user=request.user, question=post if post_type == "question" else None, story=post if post_type == "story" else None
    )
